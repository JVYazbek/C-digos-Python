...
def coleta_dados_docente(indice):
    ...
    while aulas < 0:
        print("A quantidade de aulas semanais deve ser não-negativa.")
        aulas = int(input("Quantidade de aulas semanais: "))

    valor = float(input("Custo da hora-aula (ex: 50.00): "))
    while valor < 0:
        print("O custo da hora-aula deve ser não-negativo.")
        valor = float(input("Custo da hora-aula (ex: 50.00): "))
...
def execucao_principal():
    ...
    while indice < len(lista_docentes):
        ...
        print("Remuneração Total (Salário Bruto): R$", remuneracao_total) 
        indice += 1
